...
create_box_plot <- function(pDataframe, pCategorical, pNumerical, pTitle, pCategoricalDescription, pNumbericDescription,Category)
{
  # Boxplot of scores and Test Prep by Gender
  b <- ggplot(pDataframe, aes(pCategorical, pNumerical,color=Category))
  b <- b + geom_boxplot()
  b <- b + ggtitle(pTitle)
...
#create_bar_plot <- function(pDataframe, pCategorical, Category, pNumerical, pTitle, pCategoricalDescription, pNumbericDescription)
#{
  # Barplot of scores and Test Prep by Gender
#  b <- ggplot(pDataframe, aes(x=pCategorical,y= pNumerical,fill=Category))
#  b <- b + geom_col(position='dodge',col='black')
#  b <- b + ggtitle(pTitle)
#  b <- b + xlab(pCategoricalDescription) + ylab(pNumbericDescription)
#  b <- b + theme_hc()+
#    scale_fill_manual(values=ggthemes_data$hc$darkunica[c(4,2)])+
#    geom_text(aes(label=pNumerical),position=position_dodge(width=1), vjust =-.5)
#  b
  
  
  
# pDataframe<-mutate(pDataframe,total_score=math.score+reading.score+writing.score)
#  pDataframe<-group_by(pDataframe,parental.level.of.education	, pCategorical)
#  pDataframe<-summa
#  pDataframe %>% 
#    summarise(median_score = median(total_score)) %>% 
#    ggplot(aes(x = parental.level.of.education, y = median_score, fill = pCategorical))+
...
#    geom_text(aes(label=median_score),position=position_dodge(width=1), vjust =-.5)
  ...
  #  mutate(total_score = math.score+reading.score+writing.score) %>% 
   # group_by(education = pCategorical, Category) %>% 
  #  summarise(median_score = median(total_score)) %>% 
   # ggplot(aes(x = education, y = median_score, fill = Category))+
  #  geom_col(position='dodge',col='black')+
   # theme_hc()+
  #  scale_fill_manual(values=ggthemes_data$hc$darkunica[c(4,2)])+
   # labs(x='',y='Median Total Score', fill = 'Lunch')+
...
#create_bar_plot <- function(pDataframe, pCategoricalVar, pMainTitle, pXtitle, pYtitle)
#{
  
 # pDataframe %>% 
  #  count(pCategoricalVar,race.ethnicity) %>% 
   # ggplot(aes(x=pCategoricalVar,y=n,fill=race.ethnicity))+
...
create_count_histogram_With_Gradient_filling <- function(pDataframe, Category,pNumerical, pMainTitle, pYtitle, pXtitle)
{
  ### Bar for categorical variables
  #ggplot(data=pDataframe, aes(x = pCategoricalVar)) +
   # geom_bar(
    ...
    #scale_fill_gradient("Count", low="green", high = "red") +         ###  alternative fill setting colours according to count
    #labs(title = pMainTitle, x = pXtitle, y = pYtitle)
  p1 <- ggplot(pDataframe, aes(pNumerical)) + geom_histogram(binwidth=5, color="white", aes(fill=Category))
  p1 <- p1 + xlab(pXtitle) + ylab(pYtitle) + ggtitle(paste0(pXtitle," by ",pYtitle))+ theme(plot.title = element_text(size=35))
  p1
    ...
    sidebarLayout(
      ...
      sidebarPanel(
        ...
        conditionalPanel(
          ...
          selectizeInput(inputId = "genAll1", label = "Select Categorical", choices = "")
        ),
        conditionalPanel(
          condition = "input.plottype == 'Scatterplot' || input.plottype == 'Histogram' || input.plottype == 'Correlogram' || input.plottype == 'Barplot' || input.plottype == 'Hypothesis' || input.plottype == 'Boxplot'",
      ...
      #   actionButton(inputId = "runBarplot", label = "Run Barplot")
      # ),
        conditionalPanel(
...
server <- shinyServer(function(input, output, session) {
  ...
  observeEvent(input$plottype, {
    ...
    {
      updateSelectizeInput(session = session, inputId = "genAll", label = "Histogram Category", choices = (dataread_str()[dataread_str()[,2] == "factor", 1]), options = list(maxItems = 1, minItems = 1))
      updateSelectizeInput(session = session, inputId = "genNumber", label = "Histogram Variable", choices = (dataread_str()[dataread_str()[,2] != "factor", 1]), options = list(maxItems = 1, minItems = 1))
  ...
  #  {
  #    #updateSelectizeInput(session = session, inputId = "genAll", label = "Barplot Category", choices = (dataread_str()[dataread_str()[,2] == "factor", 1]), options = list(maxItems = 1, minItems = 1))
  #    updateSelectizeInput(session = session, inputId = "genAll",label = "Barplot Category", choices = (dataread_str()[dataread_str()[,2]=="factor", 1]), options = list(maxItems = 1, minItems = 1))
  #    updateSelectizeInput(session = session, inputId = "genAll1",label = "Barplot Category1", choices = (dataread_str()[dataread_str()[,2]=="factor", 1]), options = list(maxItems = 1, minItems = 1))
  #    updateSelectizeInput(session = session, inputId = "genNumber", label = "Barplot Variable", choices = (dataread_str()[dataread_str()[,2] != "factor", 1]), options = list(maxItems = 1, minItems = 1))
  ...
  #    }
    ...
    else if (input$plottype == "Boxplot")
    {
      ...
      updateSelectizeInput(session = session, inputId = "genAll1", label = "Boxplot type Category", choices = (dataread_str()[dataread_str()[,2] == "factor", 1]), options = list(maxItems = 1, minItems = 1))
  ...
  #barplotEvent <- eventReactive(input$runBarplot, {
  #  dataread()
  #})
  ...
  predictionEvent <- eventReactive(input$runPrediction, {
    mydata<-dataread()
    ### Prediction model
  ...
  output$generic <- renderPlot(
    ...
    {
      create_count_histogram_With_Gradient_filling(histogramEvent(), histogramEvent()[,input$genAll], histogramEvent()[,input$genNumber], pCaption(), input$genAll, input$genNumber)
  ...
  #  {
  #    #create_count_histogram_With_Gradient_filling(histogramEvent(), histogramEvent()[,input$genAll], histogramEvent()[,input$genNumber], pCaption(), input$genAll, input$genNumber)
  #    create_bar_plot(barplotEvent(), barplotEvent()[,input$genAll],barplotEvent()[,input$genAll1], barplotEvent()[,input$genNumber], pCaption(), input$genAll, input$genNumber)
  #  }
    else if (input$plottype == "Pie")
    {
    ...
    {
      create_box_plot(boxplotEvent(), boxplotEvent()[,input$genAll], boxplotEvent()[,input$genNumber], pCaption(), input$genAll, input$genNumber,boxplotEvent()[,input$genAll1])
    }
    else NULL
  )
