...
create_count_histogram <- function(pDataframe, Category,pNumerical, pMainTitle, pYtitle, pXtitle)
{
...
server <- shinyServer(function(input, output, session) {
  ...
  pCaption <- eventReactive(input$btnLoadData, {
    if (input$dataMethod == "csv File") paste("Source :", input$file$name)
    else paste("Source :", input$URL)
  } )
  ...
  observeEvent( dataread_str(), {
      ...
      updateRadioButtons(session = session, inputId = "plottype", choices = c("Scatterplot", "Correlogram", "Histogram", "Pie", "Hypothesis", "Boxplot"), selected = "")
  ...
  observeEvent(input$plottype, {
    if (input$plottype == "") {return()}
    updateTabsetPanel(session = session, "tabsPanel", selected = "GenPlot")
  ...
  #    }
    ...
    {
      updateTabsetPanel(session = session, "tabsPanel", selected = "Hypothesis Testing")
  ...
  scatterEvent <- eventReactive(input$runScatter, {
    print("eventreactive scatterEvent")
    if (length(input$genAll) == 0 || length(input$genNumber) == 0)
    {
      return(NULL)
    }
    else
  ...
  boxplotEvent <- eventReactive(input$runBoxplot, {
    print("eventreactive boxplotEvent")
    if (length(input$genAll) == 0 || length(input$genNumber) == 0)
    {
      return(NULL)
    }
  ...
  histogramEvent <- eventReactive(input$runHistogram, {
    print("eventreactive histogramEvent")
    if (length(input$genAll) == 0)
    {
      return(NULL)
    }
    else
  ...
  correlogramEvent <- eventReactive(input$runCorrelogram, {
    print("eventreactive correlogramEvent")
    if (length(input$genNumber) == 0)
    {
      return(NULL)
    }
    else
  ...
  hypothesisEvent <- eventReactive(input$runHypothesis, {
    if (length(input$genNumber) == 0 || length(input$tail) == 0 || length(input$alpha) == 0)
    {
      return(NULL)
    }
  ...
  output$generic <- renderPlot(
    ...
    {
      create_count_histogram(histogramEvent(), histogramEvent()[,input$genAll], histogramEvent()[,input$genNumber], pCaption(), input$genAll, input$genNumber)
    }
    else if (input$plottype == "Pie")
    {
